...
## UI / Design
- Panel **Specializations** je v levém sloupci, pod sekcí Controls
- Pro každou specializaci jeden řádek se čtyřmi prvky:
  ...
  - **Level** — stepper s tlačítky `−` a `+` (rozsah 1–5); zobrazuje aktuální číslo
  - **Req.** — checkbox; pokud zaškrtnuto, každá feature musí obsahovat alespoň jeden úkol dané specializace
- Pod tabulkou nápověda: `Level: higher = must finish first. Same level = parallel. Req.: always in every backlog item (takes effect on regenerate).`

## Specification by Example

**Příklad 1: Nastavení fázového procesu (Design → Dev → QA)**
- Given: Design má level 3, FE a BE mají level 2, QA má level 1
- When: Simulace běží
...
- Given: FE a BE mají oba level 2, QA má level 1
- When: Jsou dostupní FE i BE specialisté a Design je dokončen
...
- When: Kouč změní Design na level 3
...
- Engine `tick()`: před přiřazením úkolu kontroluje `isAvailable(task, feature)` — ověřuje, zda jsou všechny úkoly vyšší úrovně v téže feature ve stavu `done`
